...
export function TestimonialSlider({ testimonials }: TestimonialSliderProps) {
  ...
  const onTouchEnd = () => {
    ...
    if (isLeftSwipe && currentIndex < Math.max(0, testimonials.length - visibleCount)) {
      handleNext();
    }
      ...
      >
        ...
        >
          {testimonials.map((testimonial) => (
            <div key={testimonial.id} className="flex-shrink-0 w-full sm:w-1/2 lg:w-1/3 xl:w-1/4 p-2">
